...
findcol:{[dbdir;table;col] / findcol[`:/k4/data/taq;`trade;`iz]
 fndcols:find1col[;col]each allpaths[dbdir;table];
 if[not any fndcols;'"Requested column not found in all partitions, see log output above"]}
...
.dbmaint.addcol[`:.;`trade;`num;10]
.dbmaint.addcol[`:.;`trade;`F;`test]
.dbmaint.delete1col[`:./2000.10.02/trade;`F]
.dbmaint.fixtable[`:.;`trade;`:./2000.10.03/trade]
.dbmaint.reordercols[`:.;`quote;except[2 rotate cols quote;`date]]
.dbmaint.clearattrcol[`:.;`trade;`sym]
.dbmaint.setattrcol[`:.;`trade;`sym;`p]
.dbmaint.castcol[`:.;`trade;`time;`second]
.dbmaint.renamecol[`:.;`trade;`price;`PRICE]
`PRICE`size .dbmaint.renamecol[`:.;`trade]'`p`s
