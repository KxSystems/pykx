...
def test_creation(kx):
    ...
    db.create(tab, 't', format='splayed')
    assert db.tables == ['t']
...
def test_create_errors(kx):
    ...
    with pytest.raises(kx.QError) as err:
        db.create(tab, 't', format='unsupported')
...
def test_list(kx):
    ...
    assert db_cols == ['date', 'ti', 'p', 'sz', 'sym']
    with pytest.raises(kx.QError) as err:
...
def test_column_add(kx):
    db = kx.DB()
    db.load('splay_db')
    assert ['date', 'ti', 'p', 'sz', 'sym'] == db.list_columns('t')
    db.add_column('t', 'vol', kx.IntAtom.null)
    db_cols = db.list_columns('t')
    assert ['date', 'ti', 'p', 'sz', 'sym', 'vol'] == db_cols
...
def test_column_reorder(kx):
    db = kx.DB()
    db.load('splay_db')
    db.reorder_columns('t', ['vol', 'sym', 'sz', 'p', 'ti', 'date'])
    assert ['vol', 'sym', 'sz', 'p', 'ti', 'date'] == db.list_columns('t')
...
def test_column_rename(kx):
    ...
    assert ['vol', 'sym', 'sz', 'price', 'ti', 'date'] == db.list_columns('t')
    with pytest.raises(kx.QError) as err:
...
def test_column_delete(kx):
    ...
    assert ['sym', 'sz', 'price', 'ti', 'date'] == db.list_columns('t')
    with pytest.raises(kx.QError) as err:
...
def test_column_find(kx):
    db = kx.DB()
    db.load('splay_db')
    assert db.find_column('t', 'price') # noqa: E711
    assert not db.find_column('t', 'no_col')
...
def test_column_copy(kx):
    ...
    assert ['sym', 'sz', 'price', 'ti', 'date', 'size'] == db.list_columns('t')
...
def test_table_rename(kx):
    ...
    assert type(db.trades) == kx.SplayedTable # noqa: E721
...
def test_compress(kx):
    ...
    db.create(qtab, 'comptab', format='splayed', compress=compress)
    assert zd_cache == kx.q.z.zd
    compress_info = kx.q('-21!`:./comptab/col1')
